...
export const useOnSubmit = (context: Context, navigate, dispatch, overrideSavedMessage?: string, overrideParameters?: Parameters) => {
    return useCallback(async (messageString: string) => {
        ...
        const id = await context.onNewMessage(messageString, overrideSavedMessage, overrideParameters);
